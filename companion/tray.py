...
class BongoCatSystemTray:
    ...
    def start_ui_loop(self):
        """Run all Tkinter work on one dedicated UI thread."""
    ...
    def _ui_thread_main(self):
        try:
            ...
            self._ui_ready.set()
            self.tk_root.after(25, self._pump_ui_commands)
            self.tk_root.mainloop()
        except Exception as exc:
            print(f"Settings UI failed to start: {exc}")
            self._ui_ready.set()
        finally:
            self.tk_root = None
    ...
    def _pump_ui_commands(self):
        """Execute UI work only from Tkinter's owning thread."""
        try:
            while True:
                command = self._ui_commands.get_nowait()
                if command == "settings":
                    self._show_settings_on_ui_thread()
                elif command == "quit":
                    self.tk_root.quit()
                    return
        except queue.Empty:
    ...
    def show_settings(self, item=None):
        ...
        try:
            if self._settings_process and self._settings_process.poll() is None:
                self.show_notification("Bongo Cat", "Settings is already open.")
                return
            base_path = Path(
                getattr(sys, "_MEIPASS", Path(__file__).resolve().parent)
            )
            script_path = base_path / "settings.ps1"
            if not script_path.exists() or not self.config:
                raise FileNotFoundError("settings window resources are missing")
            status_path = self.config.config_dir / "desktop_status.json"
            self._settings_process = subprocess.Popen(
                [
                    "powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass",
                    "-File", str(script_path),
                    "-ConfigPath", str(self.config.config_file),
                    "-StatusPath", str(status_path),
                ],
                creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0),
            )
        except Exception as exc:
            self.show_notification("Bongo Cat", f"Could not open Settings: {exc}")

    def _show_settings_on_ui_thread(self):
        try:
            ...
            if not self.settings_gui:
                self.settings_gui = BongoCatSettingsGUI(
                    ...
                    parent_root=self.tk_root,
                )
            self.settings_gui.show()
        except Exception as exc:
            print(f"Failed to create settings window: {exc}")
