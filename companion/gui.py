...
class BongoCatSettingsGUI:
    ...
    def create_window(self):
        ...
        if self.config:
            self.original_config = {
                ...
                'app_animations': self.config.get_app_animation_settings().copy()
            }
    ...
    def create_gui_content(self):
        ...
        try:
            ...
            self.create_app_animations_tab(notebook)
    ...
    def create_media_tab(self, notebook):
        """Show live media diagnostics and local Spotify pacing controls."""
        frame = ttk.Frame(notebook)
        notebook.add(frame, text="Media")
        main_frame = ttk.Frame(frame)
        main_frame.pack(fill='both', expand=True, padx=20, pady=20)

        spotify_group = ttk.LabelFrame(main_frame, text="Spotify API - live status", padding=12)
        spotify_group.pack(fill='x', pady=(0, 15))
        self.widgets['spotify_status_label'] = ttk.Label(
            spotify_group, text="Checking local status...", wraplength=490,
        )
        self.widgets['spotify_status_label'].pack(anchor='w')
        ttk.Label(
            spotify_group,
            text="Refresh reads local status only; it never sends a Spotify request.",
            wraplength=490, foreground='gray',
        ).pack(anchor='w', pady=(8, 0))
        ttk.Button(
            spotify_group, text="Refresh status", command=self.refresh_media_status,
        ).pack(anchor='w', pady=(10, 0))

        pacing_group = ttk.LabelFrame(main_frame, text="Adaptive API limit", padding=12)
        ...
        ttk.Label(pacing_group, text="Fastest (s)").grid(row=1, column=2, sticky='w', padx=(14, 0))
        ttk.Label(pacing_group, text="Idle (s)").grid(row=1, column=4, sticky='w', padx=(14, 0))
        self.widgets['api_initial_interval'] = tk.DoubleVar(value=3.0)
        self.widgets['api_min_interval'] = tk.DoubleVar(value=1.0)
        self.widgets['api_idle_interval'] = tk.DoubleVar(value=10.0)
        ttk.Spinbox(pacing_group, from_=0.5, to=60.0, increment=0.5, width=7,
                    textvariable=self.widgets['api_initial_interval'],
                    command=self.on_setting_changed).grid(row=2, column=0, sticky='w')
        ttk.Spinbox(pacing_group, from_=0.5, to=60.0, increment=0.5, width=7,
                    textvariable=self.widgets['api_min_interval'],
                    command=self.on_setting_changed).grid(row=2, column=2, sticky='w', padx=(14, 0))
        ttk.Spinbox(pacing_group, from_=1.0, to=300.0, increment=1.0, width=7,
                    textvariable=self.widgets['api_idle_interval'],
                    command=self.on_setting_changed).grid(row=2, column=4, sticky='w', padx=(14, 0))
        ttk.Label(pacing_group, text="Artwork retries").grid(row=3, column=0, sticky='w', pady=(10, 0))
        self.widgets['artwork_retry_attempts'] = tk.IntVar(value=5)
        ...
        self.widgets['windows_media_status_label'] = ttk.Label(
            windows_group, text="Checking Windows media...", wraplength=490,
        )
        self.widgets['windows_media_status_label'].pack(anchor='w')
        self.widgets['resource_status_label'] = ttk.Label(
            windows_group, text="Checking BongoDesk resource use...", wraplength=490,
        )
        self.widgets['resource_status_label'].pack(anchor='w', pady=(8, 0))
        ttk.Label(
            windows_group,
            text=("The physical display already shows total PC CPU/RAM. "
                  "This window also shows BongoDesk's own CPU and RAM use."),
            wraplength=490, foreground='gray',
        ).pack(anchor='w', pady=(8, 0))
        self.refresh_media_status(schedule=True)

    def refresh_media_status(self, schedule=False):
        """Update local diagnostics without making a network request."""
        spotify_label = self.widgets.get('spotify_status_label')
        windows_label = self.widgets.get('windows_media_status_label')
        resource_label = self.widgets.get('resource_status_label')
        if not spotify_label or not windows_label or not resource_label:
            return

        status = self.engine.get_media_status() if self.engine else {}
        ...
        if state == 'ready':
            ...
            spotify_text = "Spotify Client ID has not been configured."
        else:
            spotify_text = f"Spotify status unavailable: {spotify.get('message', 'unknown state')}"
        spotify_label.config(text=spotify_text)

        if status.get('windows_spotify_available'):
            windows_text = "Windows Spotify session is active (used instead of the Web API)."
        elif status.get('other_windows_media_available'):
            windows_text = "Another Windows player is active; Spotify API fallback stays active."
        else:
            windows_text = "No Windows Spotify session is currently being reported."
        windows_label.config(text=windows_text)

        resources = status.get('resources', {})
        display = "connected" if resources.get('display_connected') else "not connected"
        resource_label.config(text=(
            f"BongoDesk: {resources.get('app_cpu_percent', 0):.1f}% CPU, "
            f"{resources.get('app_memory_mib', 0):.1f} MiB RAM. PC: "
            f"{resources.get('system_cpu_percent', 0)}% CPU, "
            f"{resources.get('system_ram_percent', 0)}% RAM. Display: {display}."
        ))
        if schedule and self.window and self.window.winfo_exists():
            self.window.after(1000, lambda: self.refresh_media_status(schedule=True))

    def create_app_animations_tab(self, notebook):
        """Create lightweight foreground-app animation rules."""
        frame = ttk.Frame(notebook)
        notebook.add(frame, text="Apps")
        main_frame = ttk.Frame(frame)
        main_frame.pack(fill='both', expand=True, padx=20, pady=20)

        self.widgets['app_animations_enabled'] = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            main_frame,
            text="Choose cat animation from the active Windows app",
            variable=self.widgets['app_animations_enabled'],
            command=self.on_setting_changed,
        ).pack(anchor='w', pady=(0, 12))

        ttk.Label(
            main_frame,
            text="One rule per line: process or window text = animation",
        ).pack(anchor='w')
        rules_text = tk.Text(main_frame, height=13, wrap='none', font=('Consolas', 10))
        rules_text.pack(fill='both', expand=True, pady=(6, 10))
        rules_text.bind('<KeyRelease>', lambda _event: self.on_setting_changed())
        self.widgets['app_animation_rules'] = rules_text

        ttk.Label(
            main_frame,
            text=("Animations: IDLE_1, IDLE_2, IDLE_3, IDLE_4, BLINK, "
                  "EAR_TWITCH, TYPING_SLOW, TYPING_NORMAL, TYPING_FAST, HAPPY"),
            wraplength=430,
            foreground='gray',
        ).pack(anchor='w')
    ...
    def load_current_settings(self):
        ...
        try:
            ...
            spotify = self.config.get_setting('spotify') or {}
            self.widgets['api_initial_interval'].set(
                spotify.get('api_initial_interval_seconds', 3.0)
            )
            self.widgets['api_min_interval'].set(
                spotify.get('api_min_interval_seconds', 1.0)
            )
            ...
            app_animations = self.config.get_app_animation_settings() or {}
            self.widgets['app_animations_enabled'].set(
                app_animations.get('enabled', False)
            )
            rules_text = self.widgets['app_animation_rules']
            rules_text.delete('1.0', tk.END)
            for rule in app_animations.get('rules', []):
                rules_text.insert(
                    tk.END,
                    f"{rule.get('match', '')} = {rule.get('animation', 'IDLE_1')}\n",
                )
    ...
    def apply_settings(self):
        ...
        try:
            ...
            app_rules = []
            rules_value = self.widgets['app_animation_rules'].get('1.0', tk.END)
            for line_number, raw_line in enumerate(rules_value.splitlines(), 1):
                line = raw_line.strip()
                if not line or line.startswith('#'):
                    continue
                if '=' not in line:
                    raise ValueError(f"App rule line {line_number} needs an = sign")
                match, animation = (part.strip() for part in line.split('=', 1))
                animation = animation.upper()
                if not match or animation not in valid_animations:
                    raise ValueError(f"Invalid app rule on line {line_number}: {raw_line}")
                app_rules.append({'match': match.lower(), 'animation': animation})
            self.config.set_setting(
                'app_animations', 'enabled',
                self.widgets['app_animations_enabled'].get(),
            )
            self.config.set_setting('app_animations', 'rules', app_rules)
